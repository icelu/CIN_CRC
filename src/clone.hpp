...
// Node of a binary lineage tree, used for sampling from two sides of the tree
struct node {
    int data;
    node* left;
    node* right;
...
used in simulating multi-region samples
*/
class Sample{
public:
    int sample_ID;
...
public:
    ...
    ~Clone(){
        ...
        if(cells.size() > 0){   // when all cells are tracked
      ...
      */
      void grow_with_cnv_cmpl(const Cell_ptr start_cell, const Model& model, int Nend, node* root, int loc_type, double leap_size=0, int track_lineage = 0, int multiple_output = 0, int verbose = 0, int restart = 1, double tend = DBL_MAX){
          ...
          while(this->curr_cells.size() < Nend) {
              ...
              if(multiple_output == 1){    // sample at intermediate times
              ...
              if(rb < rbrate){
                  if(verbose > 1){
                    ...
                    for(auto c : this->curr_cells){
                      ...
                      // cout << " " << c->num_mut << ";" << "," << c->birth_rate << "," << c->death_rate << ";";
                    }
